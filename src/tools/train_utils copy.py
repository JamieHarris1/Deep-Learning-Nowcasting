    ...
    def early_stop(self, val_loss, model):
        if val_loss < self.min_loss:
            ...
            if self.biggest_outbreak:
                torch.save(model.state_dict(), f"../src/outputs/weights/weights-{self.past_units}-{self.max_delay}-{'week' if self.weeks else 'day'}-fut{self.future_obs}-biggest{'-dow' if self.dow else ''}")
            elif self.n_training is not None:
                torch.save(model.state_dict(), f"../src/outputs/weights/weights-{self.past_units}-{self.max_delay}-{'week' if self.weeks else 'day'}-fut{self.future_obs}{'-rec' if not self.random_split else ''}{'-dow' if self.dow else ''}-{self.n_training}")
            else:
                torch.save(model.state_dict(), f"../src/outputs/weights/weights-{self.past_units}-{self.max_delay}-{'week' if self.weeks else 'day'}-fut{self.future_obs}{'-rec' if not self.random_split else ''}{'-dow' if self.dow else ''}")
...
def regen_data(dataset, train_idcs):
    with open('../config.yaml', 'r') as f:
        config = yaml.safe_load(f)
    g = torch.Generator()
    g.manual_seed(config['training']['seed'])
    return DataLoader(dataset, batch_size=config['training']['batch_size'], sampler=SubsetRandomSampler(train_idcs, generator=g))

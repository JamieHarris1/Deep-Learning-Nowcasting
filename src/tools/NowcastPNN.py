...
import torch
from src.tools.NegativeBinomial import NegBin as NB
...
class NowcastPNNDOW(nn.Module):
    ...
    def __init__(self, past_units = 40, max_delay = 40, hidden_units = [16, 8], conv_channels = [16, 1], embedding_dim = 10, load_embed = True, dropout_probs = [0.15, 0.1]):
        ...
        if load_embed:
            self.embed = nn.Embedding.from_pretrained(torch.load(f"src/outputs/weights/embedding_weights_{embedding_dim}").detach())
        else:

...
def create_reporting_delay_matrix(
        ...
        raw_data_filename, transformed_data_filename
        ): 
    ...
    raw_data_path = project_dir / "data" / "raw" / raw_data_filename

    # Read in raw data
    dengdf_raw = pd.read_csv(raw_data_path, index_col=0)
    ...
    transformed_data_path = project_dir / "data" / "transformed" / transformed_data_filename
    deng_delays.to_csv(transformed_data_path, index=False)
...
def get_dataset(weeks = False, triangle = True, past_units = 40, max_delay = 40, state = "SP", future_obs = 0, return_df = False, return_mat = False, return_number_obs = False, vector_y = False, dow = False, path = "../data/DENGSP.csv", reference_col = None, report_col = None):
